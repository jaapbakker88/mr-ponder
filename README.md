...
- **risk-first, consequence-weighted** — chunks are ordered by a risk score where
  ...
  place) with `o`, anchoring on the currently focused chunk so you keep your place.
...
- **thread navigation** — `\` searches diff bodies with in-hunk highlighting; `n`/`N`
  hop between matches without switching pane focus; `*` extracts shared identifiers
  from the current chunk so you can follow a symbol through the whole diff in seconds.
...
| `n` | add a **note** (opens `$EDITOR`); or **next pattern match** when `\` is active (works from either pane) |
| `N` | **prev pattern match** when `\` is active |
| `P` | **promote** the chunk's note to a GitLab comment (confirm `y`) |
| `t` | add a tag |
| `l` | link this chunk → pick a suggestion (`1`–`9`) or `f` to free-pick |
| `*` | pick a **shared identifier** from this chunk (exported declarations / `@@` context) → sets `\` pattern; `n`/`N` to hop through all matches |
| `/` | text filter — searches file path, context line, notes, tags, **and diff body** |
| `\` | **body pattern search** (regex, or plain substring as fallback) with in-hunk highlighting; `n`/`N` to hop; `Esc` to clear |
| `u` / `s` / `d` | unseen-only / shared-only / delta (new since last review) only |
| `e` | cycle test filter: all → hide tests → only tests |
| `i` | importers panel: list files that import this shared file (blast radius) |
| `o` | toggle order: risk ↔ file (anchors on the current chunk — you keep your place) |
...
- `*` identifier extraction is heuristic (exported declarations + `@@` context); it
  misses identifiers that are defined non-exported but referenced cross-chunk within
  the same file. Passive connection hints (show related chunks without a keypress)
